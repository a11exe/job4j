...
```
+ *@RequestBody* - для передачи тела запроса в качестве параметра в метод
```java
...
```
+ *@ResponceBody* - Аннотация показывает что данный метод может возвращать кастомный объект в виде xml, json.
...
```
+ *@PathVariable* - позволяет вводить в URI переменную пути в качестве параметра.
```java
@RequestMapping(method=RequestMethod.GET, value="/emp/{id}")
public ModelAndView getEmployee(@PathVariable String id) { … }
```

+ *@RequestParam* - связывает параметр запроса и переменную по имени. По умолчанию параметры обязательны и, если его не будет, ваш код даже не будет вызван.
```java
@PostMapping("")
